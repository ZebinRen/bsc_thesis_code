...
def random_search(model, dataset, search_parameter, fixed_parameter, addi_parameter = {}, random_times = 50,
evaluate_times = 3):
    '''
    Use random search to search for hyperparameters
    model is the model class
    dataset is the dataset used to evaluate the model
    ...
    for each_time in range(random_times):
        ...
        for each_evaluate in range(evaluate_times):
            ...
            cur_accu += test_model.test(sess, dataset['adj'], dataset['features'], dataset['val_label'], 
            dataset['val_mask'])

        cur_accu /= evaluate_times
...
def desne_search(model, best_para_input, best_accu_input, dataset, search_parameter, fixed_parameter, addi_parameter = {},
evaluate_times = 3):
    '''
